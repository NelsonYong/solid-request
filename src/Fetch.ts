...
export default class Fetch<TData, TParams extends unknown[] = any> {
  ...
  count = 0
  private servicePreset: Promise<TData> | undefined | null
  ...
  constructor(
    public serviceRef: UseRequestService<TData, TParams>,
  ...
  setServicePreset(request: Promise<TData> | null | undefined) {
    if (request)
      this.servicePreset = request
    else
      this.servicePreset = null
  }
  ...
  async runAsync(...params: TParams): Promise<TData> {
    ...
    try {
      ...
      const requestReturn = (res: any) => {
        ...
        this.setServicePreset(null)
        return formattedResult
      }

      if (!servicePromise) {
        servicePromise = this.servicePreset ?? this.serviceRef(...params)
      }
